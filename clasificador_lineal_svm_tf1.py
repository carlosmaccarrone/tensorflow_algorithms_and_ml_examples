...
x_y_all_values = np.array([ [dat[0], dat[1]] for dat in all_values ])
z_all_values = np.array([ dat[2] for dat in all_values ])

x_y_vals_train = np.array([ row for row in x_y_all_values ])
z_vals_train = np.array([ row for row in z_all_values ])
...
for _ in range(2000):
	...
	puntos_aleatorios = x_y_vals_train[rand_idx]
	clasificaciones_aleatorias = np.transpose([z_vals_train[rand_idx]])
...
predYtarget = np.c_[predicciones, z_vals_test]
...
# ######## PLOTEO ##
# ##################
x_objetivo = [ elem[0] for elem in all_values if elem[2] ==  1 ]
y_objetivo = [ elem[1] for elem in all_values if elem[2] ==  1 ]
x_trivial  = [ elem[0] for elem in all_values if elem[2] == -1 ]
y_trivial  = [ elem[1] for elem in all_values if elem[2] == -1 ]
...
ax1.contourf(termx, termy, Z, cmap=ListedColormap(['#ad301d', 'green']))
ax1.grid(True)
ax1.set_xlim([0., 1.4])
ax1.set_ylim([0.8, 4])
