...
x_all_values = np.array([ [dat[1], dat[2]] for dat in all_values ], dtype=np.float32)
y_all_values = np.array([ dat[0] for dat in all_values ], dtype=np.float32)

x_vals_train = np.array([ row for row in x_all_values ], dtype=np.float32)
y_vals_train = np.array([ row for row in y_all_values ], dtype=np.float32)
...
def model_training():
	...
	for _ in range(2000):
		...
		puntos = x_vals_train[rand_idx]
		clasificaciones = np.transpose([y_vals_train[rand_idx]])
