...
x_z_all_values = np.array([ [dat[1], dat[2]] for dat in all_values ], dtype=np.float32)
z_all_values = np.array([ dat[0] for dat in all_values ], dtype=np.float32)

x_z_vals_train = np.array([ row for row in x_z_all_values ], dtype=np.float32)
z_vals_train = np.array([ row for row in z_all_values ], dtype=np.float32)

dataLen = len(x_z_vals_train)
test_idx = np.random.choice((dataLen-14), round(dataLen*0.2), replace=False)
x_z_vals_test = x_z_all_values[test_idx]
...
def model_training():
	...
	for _ in range(2000):
		...
		puntos = x_z_vals_train[rand_idx]
...
predicciones = [ calculoSignado(x, y)[0] for x, y in x_z_vals_test ]
...
min_x, min_y = np.amin(x_z_all_values,0)
max_x, max_y = np.amax(x_z_all_values,0)
